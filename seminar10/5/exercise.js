...
/**
 * GET all courses of a specific student in a specific university
 */
app.get('/universities/:universityId/students/:studentId/enrollments', async (req, res, next) => {
  try {
    ...
    if (courses.length === 0) {
      return res.status(204).json({ message: 'No enrollments found.' });
    }
...
/**
 * GET / - Export all universities with their students, courses, and enrollments
 */
  ...
  try {
    ...
    for (const uni of universities) {
      ...
      for (const course of uni.courses) {
        const studentsInCourse = await course.getStudents({ attributes: ['id'] });
        for (const student of studentsInCourse) {
          enrollments.push({ studentId: student.id, courseId: course.id });
        }
      }
